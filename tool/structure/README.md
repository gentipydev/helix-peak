...
renders. It is offline, run by hand, and run once per protein — the `.glb`s are
committed and the app never touches this directory at runtime.

It is kept because a committed binary asset that cannot be regenerated is a
...
python3 -m venv venv && venv/bin/pip install trimesh numpy scipy

venv/bin/python tool/structure/bake.py --all        # from the project root
```
...
one `.obj` per chain — and copies the finished model into `assets/models/`.
`output/` and `venv/` are gitignored.

`assets/models/` is deliberately **not** in the pubspec's asset list. The
`.glb`s are build-time sources: `hook/build.dart` discovers every `.glb` under
`assets/` and compiles it into `flutter_scene_generated/`, and that is what
ships. Dropping a new model in is enough; there is no pubspec entry to add.
