...
baked says `scored=False` in `tool/targets.py`: `--all` passes it by, `--target`
...
```sh
python3.12 -m venv tool/.esm-venv
tool/.esm-venv/bin/python -m pip install -r tool/constraint/requirements-lock.txt
tool/.esm-venv/bin/python -m unittest discover -s tool/constraint -v
...
Region boundaries and disulfide pairs come from `tool/targets.py` (they are
...
```sh
tool/.esm-venv/bin/python tool/constraint/score_protein.py --metadata-only --all
```
...
```sh
tool/.esm-venv/bin/python -u tool/constraint/verify_cpu.py --target insulin
```
