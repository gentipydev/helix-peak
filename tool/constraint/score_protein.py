...
The sequence is never typed in: it is read out of the protein the app already
draws, `assets/mock/gene_<gene>.json`, so a constraint track cannot come to
describe a different molecule from the one under it. Region boundaries and
disulfide pairs come from `tool/targets.py` and are written into the asset,
...
partner without knowing which protein it is looking at.
    ...
    tool/.esm-venv/bin/python -u tool/constraint/score_protein.py --target lysozyme
    tool/.esm-venv/bin/python -u tool/constraint/score_protein.py --all
"""
...
from targets import BY_SLUG, ESM_CONTEXT_RESIDUES, TARGETS, Target, partition  # noqa: E402
...
def sequence_of(target: Target) -> str:
    ...
    record = json.loads((ROOT / target.mock_asset).read_text())
...
if __name__ == "__main__":
    ...
    for target in chosen:
        destination = Path(args.output or ROOT / target.constraint_asset)
