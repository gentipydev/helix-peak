"""The proteins the app ships offline, and everything the bakers need to know.

One row per protein, read by every tool under `tool/`: the mock gene record
builder, the ESM-2 constraint scorer and the structure bake. `lib/features/
gene_lookup/domain/entities/protein_catalog.dart` is the same table on the Dart
side; the two are checked against each other by `tool/check_assets.py`.
...
# budget has its introns scaled; see `tool/mock/build_gene_record.py`.
...
TARGETS: tuple[Target, ...] = (
    ...
    # what their tracks measured is in tool/constraint/verification.md.
    Target(
