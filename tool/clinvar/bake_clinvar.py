...
from check_assets import positions, bases, translate  # noqa: E402
...
def bake(target, replay: bool) -> None:
    mock = json.loads((ROOT / target.mock_asset).read_text())
    ...
    print(f'{target.gene}: {len(variants)} mapped SNVs; {excluded}; {path.relative_to(ROOT)}', flush=True)
