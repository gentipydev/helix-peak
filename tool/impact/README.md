...
One asset per gene, `assets/impact/<slug>_avi.json`: AlphaGenome Variant Impact
for every base the gene page can reach, filed under the coordinate the app
already uses. Offline like everything else under `tool/` — it runs by hand, its
output is committed, and the app never calls the Atlas at tap time.
...
```sh
uv venv tool/impact/venv --python 3.12
uv pip install --python tool/impact/venv/bin/python -r tool/impact/requirements.txt
```
...
`tool/impact/checkpoints/<slug>.jsonl` as it goes and resumes from there, so a
run interrupted by the quota picks up where it stopped. The checkpoint is
deleted once that gene's asset is written.

GENCODE answers are cached in `tool/impact/gencode/<GENE>.json` and committed,
...
9.7. `python3 tool/check_assets.py` re-checks the shipped files against the
