...
Offline, like everything else under `tool/`. Run by hand, output committed.
"""
...
DEFAULT_SKILL = ROOT.parent / ".claude" / "skills" / "alphagenome-variant-impact-score"
...
def gencode(gene: str, skill: pathlib.Path, refresh: bool = False) -> dict:
    ...
    if not script.exists():
        raise BakeError(
            ...
            f"Pass --skill, or restore {cached.relative_to(ROOT)}."
        )
